...
**Service tickets and request tracking.** Service requests, tickets, and operator workload tracking from institutional ticketing systems (TeamDynamix, ServiceNow, Jira, Zendesk) related to research administration operations. UDM does not include a ServiceRequest entity in the canonical model.

The recommended pattern for institutions that want to surface ticket activity inside RA queries: capture each ticket as an `Action` row with `Action_Type` resolving to `Service_Request`, attached to the relevant Proposal / Award / Subaward / ComplianceRequirement via the polymorphic `Related_Entity_Type` + `Related_Entity_ID`. The Action's `Title`, `Description`, `Assignee_Personnel_ID`, `Requestor_Personnel_ID`, `Due_Date`, `Action_Status`, `Priority`, `Origin`, and `Outcome_Description` columns carry the ticket's structured fields; the system the ticket lives in (Cayuse, ServiceNow, etc.) is named in the title or description. The ticket's external ID is preserved in the `Source_Record_ID` audit column and the originating system in `Source_System`. Per-ticket operational state (assigned queue, escalation level, SLA timers, internal comments) stays in the ticketing system; UDM surfaces only the RA-relevant facts.
