...
**Service tickets and request tracking.** Service requests, tickets, and operator workload tracking from institutional ticketing systems (TeamDynamix, ServiceNow, Jira, Zendesk) related to research administration operations. Ticket title, description, requestor, assignee, status, priority, response times, and links to related Awards/Proposals live in the institution's ticketing system. UDM does not include a ServiceRequest entity in the canonical model. Institutions that want to analyze workload patterns and response times alongside RA data add a local extension that references Personnel, Award, Proposal, and Action via FK and pulls ticket metadata from the upstream ticketing system. Per-ticket operational state (assigned queue, escalation level, SLA timers, internal comments) stays in the ticketing system.
